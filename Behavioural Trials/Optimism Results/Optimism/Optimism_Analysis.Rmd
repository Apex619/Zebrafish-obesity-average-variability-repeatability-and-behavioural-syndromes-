...
plot_model(model_positive_05_dur, type = "pred", terms = c("Group", "Sex"))

ggplot(data = Positive_ALL, mapping = aes(x = zone_05_dur, fill = Group )) + 
  geom_density(alpha=.3)
 #facet_grid(~Sex)

ggplot(data = Positive_ALL, mapping = aes(x = Group, y = zone_05_dur, fill = Group )) + 
    geom_boxplot()+
  facet_grid(~Sex)
...
plot(model_positive_05_dur)
qqnorm(resid(model_positive_05_dur))
summary(model_positive_05_dur)
...
ggplot(data = Negative_ALL, mapping = aes(x = zone_05_dur, fill = Group )) + 
  geom_density(alpha=.3)
 #facet_grid(~Sex)

ggplot(data = Negative_ALL, mapping = aes(x = Group, y = zone_05_dur, fill = Group )) + 
    geom_boxplot()+
  facet_grid(~Sex)
...
model_negative_05_dur <- lmer(zone_05_dur ~ Group*Sex  + (1 | Fish_ID), data = Negative_ALL)
plot(model_negative_05_dur)
qqnorm(resid(model_negative_05_dur))
summary(model_negative_05_dur)
...
ggplot(data = Mixed_ALL, mapping = aes(x = zone_05_dur, fill = Group )) + 
  geom_density(alpha=.3)
 #facet_grid(~Sex)

ggplot(data = Mixed_ALL, mapping = aes(x = Group, y = zone_05_dur, fill = Group )) + 
    geom_boxplot()+
  facet_grid(~Sex)
...
plot_model(model_mixed_05_dur, type ="int")
plot(model_mixed_05_dur)
qqnorm(resid(model_mixed_05_dur))
summary(model_mixed_05_dur)
