...
Learning_rptr_F0 <- tibble(
  ...
  ci.lb = c(rpt_learning_control$CI_emp$`2.5%`,rpt_learning_treatment$CI_emp$`2.5%`,-0.02, within_between_learning_control$CI_emp[1,1], within_between_learning_control$CI_emp[2,1],within_between_learning_treatment$CI_emp[1,1], within_between_learning_treatment$CI_emp[2,1]),
  ci.ub = c(rpt_learning_control$CI_emp$`97.5%`,rpt_learning_treatment$CI_emp$`97.5%`, 0.50, within_between_learning_control$CI_emp[1,2], within_between_learning_control$CI_emp[2,2],within_between_learning_treatment$CI_emp[1,2], within_between_learning_treatment$CI_emp[2,2]),
...
Learning_rptr_sex <- tibble(
  ...
  ci.lb = c(rpt_learning_control_male$CI_emp$`2.5%`,rpt_learning_control_female$CI_emp$`2.5%`,rpt_learning_treatment_male$CI_emp$`2.5%`,rpt_learning_treatment_female$CI_emp$`2.5%`, -0.16, -0.25),
  ci.ub = c(rpt_learning_control_male$CI_emp$`97.5%`,rpt_learning_control_female$CI_emp$`97.5%`,rpt_learning_treatment_male$CI_emp$`97.5%`,rpt_learning_treatment_female$CI_emp$`97.5%`, 0.50, 0.49),
...
Within_Between plots
```{r}
learning_within_between <- subset(Learning_rptr_F0, Learning_rptr_F0$Group %in% c("Within-Individual Variance (Control Group)","Within-Individual Variance (Treatment Group)", "Between-Individual Variance (Control Group)", "Between-Individual Variance (Treatment Group)")) #Will be used for plotting within-and between individual variance

#Creating a new column
learning_within_between$Tank <- c("Control", "Control", "Treatment", "Treatment")


#Renaming to plot within and between individual variances
learning_within_between$Condition <- c("Between-Individual Variance", "Within-Individual Variance", "Between-Individual Variance","Within-Individual Variance")



learning_within_between_personality <- ggplot(learning_within_between, aes(x=Endpoint, y=r, colour=Tank)) +
geom_errorbar(aes(ymin=ci.lb, ymax=ci.ub), width = 0.4, position = position_dodge(0.3), size=0.8) +
geom_point(aes(x = Endpoint, y = r), position = position_dodge(0.3), size=3)+
geom_hline(yintercept = 0, lty = "dotted") +
coord_flip()+
...
learning_within_between_personality
```
Individual means and plotting
```{r}
individual_means_learning <- p2cs %>%
    group_by(fishID, set, sex) %>%
    dplyr::summarize(Mean = mean(difference, na.rm=TRUE))

individual_means_learning

indiv_learning <-ggplot(individual_means_learning, aes(set, Mean, fill=set)) + 
