...
df <- data.frame(rpt_treatment_minus_rpt_control)
...
ggplot(rpt_behav_contrasts) + 
geom_pointrange(aes(x = behavior, y = mean.post, ymin = ci.lb, ymax = ci.ub))+
geom_hline(yintercept=0, linetype="dotted")+
coord_flip()

#This should essentially be the same as "social vs anxiety", "predator vs anxiety" etc.

#Basically what this plot means, is that repeatabilities between control and treatments differ (or not) between the two behaviors (anxiety response and behavioral trait) ?

```

Running rpt models overnight
```{r}
