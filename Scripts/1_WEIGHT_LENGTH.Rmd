...
### REPEATABILITY ANALYSIS FOR BODY WEIGHT AND LENGTH (body length in progress as last measurements are finalized)
...
3) FINAL MIXED MODEL USING LME FUNCTION
4) CALCULATING VARIANCE DIFFERENCES BETWEEN CONTROL AND TREATMENT GROUPS
...
rpt_weight(Weight_Length_Data) -> rpt_body_weight
rpt_body_weight

#Obtaining within-individual and between-individual variance for body weight

rpt_within_weight(Weight_Length_Data) -> within_between_body_weight
within_between_body_weight

# Calculating difference in variance between control and treatment group 
Model_Weight_1 <- lme(Weight.g. ~ Treatment_Tank*Week + Sex*Week, random = ~ 1| Fish_ID, data = Weight_Length_Data) #Mixed model without variance structure
summary(Model_Weight_1)
Model_Weight_2 <- lme(Weight.g. ~ Treatment_Tank*Week + Sex*Week, random = ~ 1| Fish_ID, weights = varIdent(form=~1|Treatment_Tank), data = Weight_Length_Data) #mixed model with variance structure varIdent
...
Model_Body_Length2 <- lmer(Fish_Length_cm ~ Treatment_Tank*Week + Sex*Week + (1 | Fish_ID) + (1 + Week|Tank), data = Weight_Length_Data) 
...
#Calculating repeatabilities using custom made functions
rpt_length(Weight_Length_Data) -> rpt_length
rpt_length

#Obtaining within-individual and between-individual variance for body weight

rpt_within_length(Weight_Length_Data) -> within_between_body_length
within_between_body_length
