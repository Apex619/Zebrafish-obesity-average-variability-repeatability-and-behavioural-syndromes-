...
Model_Weight_1 <- lme(Weight.g. ~ Treatment_Tank-1 + Sex + Week, random = ~ 1| Fish_ID, data = Weight_Length_Data) #Mixed model without variance structure
summary(Model_Weight_1)
Model_Weight_2 <- lme(Weight.g. ~ Treatment_Tank-1 + Sex + Week, random = ~ 1| Fish_ID, weights = varIdent(form=~1|Treatment_Tank), data = Weight_Length_Data) #mixed model with variance structure varIdent
