...
vars_behavs <- bind_cols(vars_behavs_list) #HAMZA: not binding due to unequal length? Not sure why Social has 357 rows and the rest 358?

# ROSE2: delete this bit now the error is fixed (keep code tidy)
# ROSE: investigating where this error has come from, by adding a column for unique fish sessions etc
# func_variable_behav <- function(behav = behavs[1]){
#     behav_vars <- Personality_df[Personality_df$Behaviour == behav,vars] # getting just the variables for that behaviour
#     names(behav_vars) <- paste(names(behav_vars), behav, sep = "_") # renaming variables so they say which behaviour they relate to
#     behav_vars$fish_session_ID <- Personality_df[Personality_df$Behaviour == behav,"fish_session_ID"]
#     return(behav_vars)
# }
# vars_behavs_list <- lapply(behavs, function(x) func_variable_behav(x))
# # now need to find which trial_ID is not present in all of these behaviours
# trial_ID_list <- lapply(vars_behavs_list, function(x) x$fish_session_ID)
# trial_ID_list[[1]] == trial_ID_list[[2]] # these should be all TRUE, so something has gone wrong
# Personality_df %>% group_by(fish_session_ID) %>% tally() %>% group_by(n) %>% tally() # there should be FOUR repeats of fish_session_ID, not three
# # is this problem present in the imported datafame?
# Personality <- Personality %>% mutate(fish_session_ID = paste(Fish_ID, Date, Session, Camera, Tank)) %>% arrange(fish_session_ID)
# Personality %>% group_by(fish_session_ID) %>% tally() %>% group_by(n) %>% tally() # NO
# # FOUND THE PROBLEM:
# unique(Personality$Time) #"6-Sep" "13-16" "20-23" "27-30"
# # so when we did Personality$phase <-  factor(Personality$Time, levels = c("6-9", "13-16", "20-23", "27-30")), the 6-9 phase was excluded
...
# ROSE2: checking above
Post_assay <- read.csv("../Data/Personality/Post-assay.csv")
Personality_wide2 <- left_join(Post_assay, Personality_wide)
names(Personality_wide2) # ROSE2: we don't want these "Phase columns"
Personality_wide2 <- Personality_wide2[,!(str_detect(names(Personality_wide2), "Phase"))]
# double-checking matching was done correctly
Personality_wide2$session_ID = paste(Personality_wide2$Fish_ID, Personality_wide2$Date, Personality_wide2$Session, Personality_wide2$Camera) 
wide_ID = paste(Personality_wide$Fish_ID, Personality_wide$Date, Personality_wide$Session, Personality_wide$Camera) 
postassay_ID = paste(Post_assay$Fish_ID, Post_assay$Date, Post_assay$Session, Post_assay$Camera)
Personality_wide2$session_ID == wide_ID
Personality_wide2$session_ID == postassay_ID # these are all true, so Personality_wide2 is arranged in the order of the postassay dataframe
Personality_wide$tot_dist_Aggression[match(postassay_ID, wide_ID)] == Personality_wide2$tot_dist_Aggression # correct
Personality_wide <- Personality_wide2
```
...
func_behav_sqrt <- function(behav = behavs[1]){
  ...
  func_var <- function(var = vars[1]){
    mod <- lmer(sqrt(Personality_wide[,paste(var,behav,sep="_")]) ~ Group*Sex + (1 | Fish_ID), data = Personality_wide) 
    residuals <- resid(mod)
    VC <- data.frame(VarCorr(mod))
    rpt <- round(VC$vcov[1]/sum(VC$vcov),2)
    sw <- shapiro.test(residuals)
...
func_behav_log <- function(behav = behavs[1]){
  
  func_var <- function(var = vars[1]){
    mod <- lmer(log(Personality_wide[,paste(var,behav,sep="_")]+1) ~ Group*Sex + (1 | Fish_ID), data = Personality_wide)  # ROSE2: this is common to see in papers but if you care about variance estimation then it's best to avoid log transformations when you have zeroes in your data.
    residuals <- resid(mod)
    VC <- data.frame(VarCorr(mod))
    rpt <- round(VC$vcov[1]/sum(VC$vcov),2)
    sw <- shapiro.test(residuals)
...
rpt_resids_log <- bind_rows(lapply(behavs, function(x) func_behav_log(x))) # log transformed

# restricting to just values of W > 0.9 and seeing which variables are represented for all behaviours
rpt_resids %>% filter(W>0.9) %>% group_by(var) %>% tally()

# only mean_speed and tot_dist are there for everything
# zone_near_dur there for 4/5

rpt_resids_sqrt %>% filter(W>0.9) %>% group_by(var) %>% tally()

# zone_510_dur, zone 2025_dur, zone_middle_dur and zone_far_dur there for everything

rpt_resids_log %>% filter(W>0.9) %>% group_by(var) %>% tally()
#tot_dist_05 there for 5/5
```

## To-do:   
### Add 'Activity' behaviour into wide dataframe, from no-stimulus periods
- do you have the pre-stimulus periods in addition to the post-assay period? If so, can average the behaviours from those 5 periods?   
- acclimation period should be excluded  

#HAMZA: I don't have the pre-stimulus periods :/ I can get it from Ethovision but will take some time to ensure I do it correctly. 

#UPDATE: Activity has been added into the dataframe and have ensured to include "Acitvity" as part of our "behavs" list

### Try sqrt-transformation to see if one of the stimulus variables will be suitable for all four behaviours   
- Modify func_behav_raw to func_behav_sqrt, with the response variable = square-root transformed   
- Does normality of residuals for zone_05_dur or zone_near_dur improve for any of the behaviours?   

### Run lmer models for chosen behaviour   
```{r}
#ROSE2: To-do:
# - Write a function that will input your variable name and output the model
# - use lapply to apply that function across all behaviours
# - add more notes/annotation justifying your chosen variables

# 05 duration
lmer_zone_05_dur_Aggression <- lmer(zone_05_dur_Aggression ~ Group*Sex + (1 | Fish_ID), data = Personality_wide) 
hist(resid(lmer_zone_05_dur_Aggression))
summary(lmer_zone_05_dur_Aggression)
tab_model(lmer_zone_05_dur_Aggression)


lmer_zone_05_dur_Novel <- lmer(log(zone_05_dur_Novel+1) ~ Group*Sex + (1 | Fish_ID), data = Personality_wide)
hist(resid(lmer_zone_05_dur_Novel))
summary(lmer_zone_05_dur_Novel)

lmer_zone_05_dur_Social <- lmer(sqrt(zone_05_dur_Social) ~ Group*Sex + (1 | Fish_ID), data = Personality_wide) 
hist(resid(lmer_zone_05_dur_Social))
summary(lmer_zone_05_dur_Social)

lmer_zone_05_dur_Predator <- lmer(sqrt(zone_05_dur_Predator) ~ Group*Sex + (1 | Fish_ID), data = Personality_wide) 
hist(resid(lmer_zone_05_dur_Predator))
summary(lmer_zone_05_dur_Predator)

lmer_zone_05_dur_Activity <- lmer(sqrt(zone_05_dur_Activity) ~ Group*Sex + (1 | Fish_ID), data = Personality_wide) 
hist(resid(lmer_zone_05_dur_Activity))
summary(lmer_zone_05_dur_Activity)



# near duration

lmer_zone_near_dur_Aggression <- lmer(log(zone_near_dur_Aggression+1) ~ Group*Sex + (1 | Fish_ID), data = Personality_wide) 
hist(resid(lmer_zone_near_dur_Aggression))
summary(lmer_zone_near_dur_Aggression)


lmer_zone_near_dur_Novel <- lmer(log(zone_near_dur_Novel+1) ~ Group*Sex + (1 | Fish_ID), data = Personality_wide) 
hist(resid(lmer_zone_near_dur_Novel))
summary(lmer_zone_near_dur_Novel)

lmer_zone_near_dur_Social <- lmer(sqrt(zone_near_dur_Social) ~ Group*Sex + (1 | Fish_ID), data = Personality_wide) 
hist(resid(lmer_zone_near_dur_Social))
summary(lmer_zone_near_dur_Social)

#Negatively skewed distributions

lmer_zone_near_dur_Predator <- lmer(sqrt(zone_near_dur_Predator) ~ Group*Sex + (1 | Fish_ID), data = Personality_wide) 
hist(resid(lmer_zone_near_dur_Predator)) # normally distributed
summary(lmer_zone_near_dur_Predator)

lmer_zone_near_dur_Activity <- lmer(sqrt(zone_near_dur_Activity) ~ Group*Sex + (1 | Fish_ID), data = Personality_wide) 
hist(resid(lmer_zone_near_dur_Activity)) 
summary(lmer_zone_near_dur_Activity)
...
- fitted = the actual measured behaviours
...
# example:
mod = lmer_zone_05_dur_Aggression
