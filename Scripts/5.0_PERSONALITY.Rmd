...
fitted_predicted$fitted<- Personality_wide[,variable] # ROSE TO-DO: you need to fix this for novel. As we have taken the square-root of novel in the model, we need ot take the square-root of novel here too
...
names(blups) # Double checking to make sure we have renamed correctly
 ...
 fish_ID <- as.character(names(blups)) #converting to characters as we did above
  ...
  pos <- match(fitted_fish, fish_ID) # for match, the first argument is the reference, and the second argument is the one you are retrieving an index for (so on line 364, it is backwards)
  ...
  male <- ifelse(fitted$Sex=="Male",1,0)
  treatment <- ifelse(fitted$Group=="Treatment",1,0)
  treatment.male <- ifelse(fitted$Group=="Treatment"&fitted$Sex=="Male",1,0)
  ...
  b3 <- post_means["b_GroupTreatment:Sexmale"]
