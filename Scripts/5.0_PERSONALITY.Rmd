...
#HAMZA: We need to extract posterior means for every brms model in the list correct? If so, here's the first function for posteriors

func_mod_posterior <- function(variable = behav_vars[1]){
  mod <- mod_list_brms[[variable]]
  mod_posterior <- as.data.frame(mod) # ROSE: you want to tidy this up and only take the population parameters
  # excluding the individual differences
  # use: names(mod_posterior) and str_detect to remove all the columns that contain individual differences (i.e. that contain Fish_ID)
  # remove that lp__ column at the end
  # rename the coefficients to make them easier to call later. Can name them the same as however you write the model (e.g., lines 219-221, B0, B1, B2, B3, sigma_ID, sigma_e)
  
  return(mod_posterior)
}

mod_post_brms <- lapply(behav_vars, function(x) func_mod_posterior(x))
names(mod_post_brms) <- behav_vars
  
# and second function for post_means
func_post_means_brms <- function(variable = behav_vars[1]){
  mod <- mod_post_brms[[variable]] # this returns an error for me
  
  post_means <- apply(mod, 2, function(x) round(mean(x),4))
  return(post_means)
}

post_means_brms <- lapply(behav_vars, function(x) func_post_means_brms(x))
names(post_means_brms) <- behav_vars

# #Function for (1) the posterior distributions of population parameters and  (2) the fitted and predicted values for posterior means.
...
func_process_mod <- function(variable = behav_vars[1]){
  ...
  # remove that lp__ column at the end
  # rename the coefficients to make them easier to call later. Can name them the same as however you write the model (e.g., lines 219-221, B0, B1, B2, B3, sigma_ID, sigma_e)
  # call this new dataframe pop_posterior
  
  # next, get the posterior means, as you did above
   post_means <- apply(mod_posterior, 2, function(x) mean(x)) # except we don't found because we're using these in subsequent calcualtions
  ...
  # to make fitted_predicted, the logic is as follows:
   ...
   # to do this calculation, make three more volumns from 'fitted': a column of 1s and 0s for treatment,a column of 1s and 0s for male, and a column of individual_difference, which you will get by matching the Fish_ID in each row to the mean BLUP from post_means
