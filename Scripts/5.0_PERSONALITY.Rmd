...
func_mod <- function(var,squareroot = F, df = Personality_wide){
  ...
  plot <- hist(residuals(mod)) # to visulaize histogams for checking normality
  sum <- summary(mod) #obtain summary of the models
  lt = list(plot, sum) # I am struggling with this section as I want to return both the plot and sumary but it doesn't seem to be working correctly
  return(lt)
...
mod_list <- lapply(tot_dist, function(x) func_mod(x)) #using lapply to apply function to all behaviors...I get the histograms when I run lapply, but when I open "mod_list" I only see the summaries of the models
...
func_fitted_predicted <- function(mod_name){
