...
Personality_df <- Personality_df %>% mutate(fish_session_ID = paste(Fish_ID, Date, Session, Camera, Tank)) %>% arrange(fish_session_ID)
...
info <- c("Time", "Day", "Date", "Video",   "Arena",   "Sex", "Mark", "Session", "Camera",  "Tank", "Group",  "Fish_ID", "Notes")
...
func_behav_log <- function(behav = behavs[1]){
  ...
  func_var <- function(var = vars[1]){
    mod <- lmer(log(Personality_wide[,paste(var,behav,sep="_")]+1) ~ Group*Sex + (1 | Fish_ID), data = Personality_wide) 
    residuals <- resid(mod)
    VC <- data.frame(VarCorr(mod))
    rpt <- round(VC$vcov[1]/sum(VC$vcov),2)
    sw <- shapiro.test(residuals)
...
Correlation_Time_Spent_Social_Dist_Travel <- posterior_samples(brm_correlation_Social_Activity)[,13]
par(mar=c(2, 2, 3, 2))
