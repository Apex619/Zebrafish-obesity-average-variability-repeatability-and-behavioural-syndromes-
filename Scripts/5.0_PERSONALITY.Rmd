...
```{r}
#HAMZA: Main struggle is releveling the population parameters :/

#I can never remember what the betas signify, I understand we have the fixed effects and they need to be releveled ?
...
func_pop <- function(variable = behav_vars[1]){
   ...
   post <-  mod[[1]]
    names(post)
    treatment_female <- post$b1
    control_male <- post$b2
    int_treatment_male <- post$b3
    pop_param <- c("treatment_female", "control_male", "int_treatment_male")
    mean <- c(mean(treatment_female), mean(control_male), mean(int_treatment_male)) 
    ci.lb <- c(quantile(treatment_female,0.025), quantile(control_male, 0.025), quantile(int_treatment_male, 0.025)) 
    ci.ub <- c(quantile(treatment_female,0.975), quantile(control_male, 0.975), quantile(int_treatment_male, 0.975)) 
    #95% CI
