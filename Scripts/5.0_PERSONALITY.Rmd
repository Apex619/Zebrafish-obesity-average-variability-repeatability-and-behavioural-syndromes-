...
mod <- mod_processed %>% mutate(rpt_control = case_when(behaviour1_control == behav ~ rpt1_control,
...
rpt_behav_contrasts <- bind_rows(lapply(behaviors, function(x) func_rpt_contrasts(x)))
...
var_contrasts_est <- bind_rows(lapply(split(Rpt_behav_contrasts, Rpt_behav_contrasts$behaviour),
      function(x)
        func(x))) 
#plot
...
ggplot(var_contrasts_est[var_contrasts_est$parameter=="rpt_treatment_minus_control",]) + 
geom_pointrange(aes(x = behaviour, y = est, ymin = lb, ymax = ub))+
geom_hline(yintercept=0, linetype="dotted")+
coord_flip()

# ROSE: these are the same so code is working

ggplot(var_contrasts_est) + 
geom_pointrange(aes(x = behaviour, y = est, ymin = lb, ymax = ub))+
geom_hline(yintercept=0, linetype="dotted")+
coord_flip() +
  facet_wrap(~parameter, scales = "free_x") # ROSE: CIs much be so much wider for activity because of the different scale of the data. You might want to standardize the response variables before analysis.
