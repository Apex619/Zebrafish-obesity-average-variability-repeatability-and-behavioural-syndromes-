...
#Comparing Control and Treatment groups To do this, we want to subset by control and treatment groups and then run the 2x10 models again without "group" as a fixed effect
...
contrasts_var <- bivar_control_treatment %>% mutate(sigma_ID1_treatment_minus_control=sigma_ID1_treatment-sigma_ID1_control,
  ...
  select(behaviour1, behaviour2,mod_id,
...
mod <- mod_processed %>% mutate(B0_control = case_when(behaviour1_control == behav ~ B0_1_control,
                                ...
                                cor_control = cor_ID1_ID2_control,
                                ...
                                cor_treatment = cor_ID1_ID2_treatment) %>%
  filter(!is.na(B0_control))
    ...
    rpt_treatment_minus_rpt_control <- rpt_treatment - rpt_control
    correlation_control <- mod$cor_control
    correlation_treatment <- mod$cor_treatment
    correlation_control_minus_correlation_treatment <- correlation_control - correlation_treatment
    

    
  
    
df_pop <- data.frame (control_female, control_male,  control_male_minus_control_female, treatment_female, treatment_male, treatment_male_minus_treatment_female,rpt_control, rpt_treatment, rpt_treatment_minus_rpt_control,correlation_control, correlation_treatment, correlation_control_minus_correlation_treatment)
...
data$coef <- (row.names(data))
data$behavior <- behav
data$coef <- factor(data$coef, levels = c("control_female", "control_male",  "control_male_minus_control_female", "treatment_female", "treatment_male",  "treatment_male_minus_treatment_female","rpt_control", "rpt_treatment", "rpt_treatment_minus_rpt_control","correlation_control", "correlation_treatment", "correlation_control_minus_corelation_treatment"))
...
Contrasts_personality_plots <- 
  ggplot(subset(bivar_behavs_results_control_treatment_contrast, coef %in% c("correlation_treatment", "correlation_control", "rpt_control", "rpt_treatment")),
  aes(x = coef, y = mean.post, colour = behavior)) +
...
contrasts_var %>% dplyr::group_by(mod_id) %>% summarise(behav_syn_mean = mean(cor_ID1_ID2_treatment_minus_control))

df <- bind_rows(lapply(split(contrasts_var, contrasts_var$mod_id), function(x) data.frame(est=mean(x$cor_ID1_ID2_treatment_minus_control),
...
#Contrasts repeatabilites per behaviour

Contrasts_rpt <- 
  ggplot(subset(bivar_behavs_results_control_treatment_contrast, coef %in% c(	
"rpt_treatment_minus_rpt_control")),
  aes(x = coef, y = mean.post, colour = behavior)) +
  geom_errorbar(aes(ymin = ci.lb, ymax = ci.ub),width = 0.4,position = position_dodge(0.3),size = 0.8) +
  geom_point(aes(x = coef, y = mean.post),
  position = position_dodge(0.3),size = 3) +
