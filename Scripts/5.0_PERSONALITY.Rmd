...
func_process_mod <- function(variable = behav_vars[1]){
  ...
  pop_posterior <- select(mod_posterior,!contains(c("Fish_ID", "lp")))
  
  b0 <- pop_posterior[1]
  b1 <- pop_posterior[2]
  b2 <- pop_posterior[3]
  b3 <- pop_posterior[4]
  sigma <- pop_posterior[5]
  ...
  # next, get the posterior means, as you did above
   post_means <- apply(mod_posterior, 2, function(x) mean(x)) # except we don't found because we're using these in subsequent calculations; HAMZA: ???
  ...
  # to make fitted_predicted, the logic is as follows:
   ...
   #HAMZA: Having trouble getting the ID :/ I believe I'm supposed to match the ID from the mod_posterior dataframe with the fish_id's from our personality_wide dataframe, a similiar process to using the lmer models hence why I have copied and pasted below to get a better idea
