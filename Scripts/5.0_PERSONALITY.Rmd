...
tot_dist_Novel_check <- lmer(tot_dist_Novel ~ Group*Sex + (1 | Fish_ID), data = df)
...
tot_dist_Social_check <- lmer(tot_dist_Social ~ Group*Sex + (1 | Fish_ID), data = df)
...
hist(residuals(mod_list$tot_dist_Social)) #Residuals don't appear to be matching? 

#From these summaries, am confident the rest of the model summaries are matching as it is following the order of the vector, am just not sure about the residuals
...
hist(residuals(lmer_zone_05_dur_Novel))

#HAMZA2: Am just concerned regarding the residuals and how the plots don't seem to match when I compare the lapply versions to the model checks
...
#HAMZA2: Am still working on this !!
...
func_fitted_predicted <- function(mod){
  betas <- fixef(mod)
  b0 <- betas[1]
b1 <- betas[2]
b2 <- betas[3]
b3 <- betas[4]
popparameters = B0 + B1 + B2 + B3
mean(posterior$column)
+ sigma_ID
plot(predicted_lmer, predicted_brm)
mean(intercept_posterior) = B0
return(list(thing1, thing2))
get_thing1 <- func()[[1]]
get_thing2 <- func()[[2]]
}

func_fitted_predicted(lmer_zone_05_dur_Aggression)
```
