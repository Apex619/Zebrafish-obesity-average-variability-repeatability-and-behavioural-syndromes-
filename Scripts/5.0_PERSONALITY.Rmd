...
abline(v=mean(Social_Predator_cor),col="black",lwd=2)
abline(v=HPDinterval(as.mcmc(Social_Predator_cor))[1],col="black",lty=2)
abline(v=HPDinterval(as.mcmc(Social_Predator_cor))[2],col="black",lty=2)
...
test <- mod_processed %>% mutate(B0 = case_when(behaviour1 == behav ~ B0_1,behaviour2 == behav ~ B0_2 )) %>% select(behaviour1, behaviour2, B0_1, B0_2, B0)
...
bivar_models <- c() #character vector with names of data frames of the bivar model outputs
