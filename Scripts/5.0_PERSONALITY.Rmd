...
func_join_plots <- function(df1 = fitted_process_brm_list[[2]], df2 = fitted_predicted_lmer_list[[2]]){
   behav<- gsub(".*dur\\_|.*dist\\_", "", df1[[2]]$variable_name[1]) 
   plot <- ggplot()+
    theme_classic()+
    geom_point(data = df1[[2]],
  aes(x=fitted, y=predicted, color="brm"))+
geom_point(data=df2, aes(x=fitted, y=predicted, colour="lmer"))+
      labs(x=df1[[2]]$variable_name[1], #HAMZA: Again, having the same issue where I cannot seem to tell the function pull out every lmer list, rather I have assigned df2 to one particular variable and it is only using that for all plots
         title=behav)

return(plot)
}

joined_plots <- lapply(fitted_process_brm_list, function(x) func_join_plots(x))

joined_plots[3] #HAMZA: For example, it keeps using the lmer data for predator
...
# TASK TWO: calculate repeatability for all behaviors from the brm models. 
 ...
 func_var_rpt_brms <- function(variable = behav_vars[1]){
   ...
   post <-  mod[[1]]
    ...
    quantiles <- quantile(rpt,c(0.025, 0.975)) #95% CI
