...
#HAMZA: I think it was easier before as I was only working with one set, but now I am working with 2 :/

func_join_plots <- function(variable = behav_vars[1]){
...
geom_point(data=, aes(x=fitted, y=predicted, colour="lmer"))+
      labs(x=df1[[2]]$variable_name[1],
         title=behav)
...
joined_plots <- lapply(behav_vars, function(x) func_join_plots(variable = x))
...
# TASK TWO: calculate repeatability for all behaviors from the brm models. 
 ...
 func_var_rpt_brms <- function(variable = behav_vars[1]){
   ...
   post <-  mod[[1]]
    ...
    quantile_0.025 <- quantile(rpt,0.025)
    quantile_97.5 <- quantile(rpt, 0.975) 
   df2 <- data.frame(variable, mean_rpt, quantile_0.025, quantile_97.5)
    return(df2)
 }
