...
# ROSE: you might want to only use zone_05_dur for the stimulus phases, and total distance for the activity phase? 
...
fixef(lmer_zone_05_dur_Novel) == fixef(mod_list_5behav$zone_05_dur_Novel) # they're the same
...
#HAMZA2: Having trouble with the lapply because I can't figure out how to use gsub properly :/ # ROSE: didn't end up needing to use gsub (but we will later on with brms) -- see below
...
# ROSE: actually because we named the list the same as the variable names, we actually only need one argument here
# behav_vars <- c("zone_05_dur_Social", "zone_05_dur_Predator", "zone_05_dur_Novel", "zone_05_dur_Aggression", "tot_dist_Activity") 
# mod_list_5behav <- lapply(behav_vars, function(x) func_mod_behav_vars(x)) 

func_fitted_predicted <- function(variable = zone_05[1]){ #I have so far just put the top code into the function :/
...
pos <- match(Personality_wide$Fish_ID, mod_pars$fish_ID)
#(mod_pars$fish_ID[pos]==Personality_wide$Fish_ID) # all TRUE
fitted_predicted <- mod_pars[pos,] %>% mutate(fitted = variable)#This is where I'm having trouble, because it has to be unique and I'm not sure how to use gsub here :/ # ROSE: no problem here?
...
return(fitted_predicted) #return both the R2 value as well as the plot of fitted vs residuals # ROSE: work on returning un-processed things (like how we returned the model before, rather than summary(model)). You can write another function to return plots and R2, if you like. 

}

# ROSE: applying this function to our variables
fitted_predicted_list <- lapply(behav_vars, function(x) func_fitted_predicted(variable = x))

# now we can write another function to plot fitted vs. predicted and calculate R2
func_fitted_predicted_plots <- function(df = fitted_predicted_list[[1]]){
  # TO-DO: in this function we will make a ggplot with fitted vs. predicted, with the following labels:
  # x-axis says what the raw data variable was
  # y-axis is the model predictions
  # title says the behaviour (activity, aggression, etc) 
  # subtitle says the R2 value
