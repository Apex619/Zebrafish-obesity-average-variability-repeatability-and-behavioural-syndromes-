...
func_pop <- function(variable = behav_vars[1]){
   ...
   post <-  mod[[1]]
    ...
    treatment_female <- post$b1 #Need to make sure I have the correct param for labelling
    control_male <- post$b2 
    int_treatment_male <- post$b3 
    pop_param <- c("control_female", "treatment_female", "control_male", "int_treatment_male")
    mean <- c(mean(control_female), mean(treatment_female), mean(control_male), mean(int_treatment_male)) 
    ci.lb <- c(quantile(control_female,0.025), quantile(treatment_female,0.025), quantile(control_male, 0.025), quantile(int_treatment_male, 0.025)) 
    ci.ub <- c(quantile(control_female,0.975), quantile(treatment_female,0.975), quantile(control_male, 0.975), quantile(int_treatment_male, 0.975)) 
   df2 <- data.frame(variable,pop_param, mean,ci.lb, ci.ub)
    return(df2)
}
...
#Attempt at doing basic forest plots
