...
func_process_mod <- function(variable = behav_vars[1]){
  ...
  pop_posterior <- select(mod_posterior,!contains(c("Fish_ID", "lp"))) # here you are getting rid of the variance component for individual differences. Need to change Fish_ID to r_Fish_ID
  
  b0 <- pop_posterior[1]
  b0 <- pop_posterior[,"b_Intercept"] # ROSE tip: it's more reproducible to call names, rather than numbers
  b1 <- pop_posterior[2]
  b2 <- pop_posterior[3]
  b3 <- pop_posterior[4]
  sigma <- pop_posterior[5] # you've lost the variance component for individual differences. You need this to calculate repeatability. 
  
  
  #HAMZA: Is there anything that I am missing here? I tried a different way to get the columns we require
  
  # next, get the posterior means, as you did above
   post_means <- apply(mod_posterior, 2, function(x) mean(x)) # except we don't round because we're using these in subsequent calculations; HAMZA: ??? # ROSE: 'found' was a typo, I meant 'round'. Before you had round(mean(x),4)
  ...
  # to make fitted_predicted, the logic is as follows:
   ...
   #HAMZA: Having trouble getting the ID :/ I believe I'm supposed to match the ID from the post_means dataframe with the fish_id's from our personality_wide dataframe, a similiar process to using the lmer models hence why I have copied and pasted below to get a better idea
  ...
  pos <- match(fish_ID, Personality_wide$Fish_ID)
  mod_pars <- data.frame(fish_ID, b0, b1, b2, b3, treatment = ifelse(Personality_wide$Group[pos] == "Treatment",1,0), male = ifelse(Personality_wide$Sex[pos]=="male",1,0)) %>% mutate(predicted = b0 + ID + b1*treatment + b2*male + b3*(treatment*male))
