...
Model_gluc_1 <- lme(Glucose ~ Sex + Weight*Group, random = ~ 1| Fish_ID, data = Glucose_Weight) #Mixed model without variance structure
summary(Model_gluc_1)
tab_model(Model_gluc_1)
Model_gluc_2 <- lme(Glucose ~ Sex + Weight*Group, random = ~ 1| Fish_ID, weights = varIdent(form=~1|Group), data = Glucose_Weight) #mixed model with variance structure varIdent
