...
Week_22 <- subset(Data, Data$Week == "22")
```
...
```{r}
#BMI
Summary_BMI_Treatment_22 <- summarySE(Week_22, measurevar="BMI", groupvars=c("Week","Treatment_Tank", "Sex"))
knitr::kable(
  Summary_BMI_Treatment_22[1:6, ], 
  caption = "Summary_BMI_Treatment"
)

pd <- position_dodge(0.1) # move them .05 to the left and right

plot_bmi_22 <- ggplot(Summary_BMI_Treatment_22, aes(x=Treatment_Tank, y=BMI, fill=Treatment_Tank)) + 
  geom_errorbar(aes(ymin=BMI-se, ymax=BMI+se), width=.1, position=pd) +
  geom_col(position=pd) +
  labs(title = "Obesogenic vs. Control Diet",
...
Summary_K_Treatment_22 <- summarySE(Week_22, measurevar="K.Index", groupvars=c("Week","Treatment_Tank", "Sex"))
knitr::kable(
  Summary_K_Treatment_22[1:6, ], 
  caption = "Summary_BMI_Treatment"
)

pd <- position_dodge(0.1) # move them .05 to the left and right

plot_K_22 <- ggplot(Summary_K_Treatment_22, aes(x=Treatment_Tank, y=K.Index, fill=Treatment_Tank)) + 
  geom_errorbar(aes(ymin=K.Index-se, ymax=K.Index+se), width=.1, position=pd) +
  geom_col(position=pd) +
  labs(title = "Obesogenic vs. Control Diet",
              subtitle = "Fish K Index after 22 weeks")+
  facet_grid(~Sex)
